...
const {
    ...
    J7_INFILL, J7_BUILD_PLATE_MM, J7_FILAMENT_DENSITY,
...
function sizingSection() {
    ...
    const plateMm = J7_BUILD_PLATE_MM;
    const plateIn = plateMm.map(mm => Math.round(mm / 25.4 * 10) / 10);

    // The worked example: a life-size prop pistol.
    const dims = [30, 15, 4];
...
same way the estimator's "describe it" option does, and show the working in a
line or two.
...
5. The printers build up to ${plateMm.join(' x ')} mm (${plateIn.join(' x ')} in). If the longest side
   is bigger, say it would have to be printed in pieces and joined, and that
   Thomas confirms whether that works - do not promise it.
...
Worked example, computed with the site's own formula - a life-size prop pistol
